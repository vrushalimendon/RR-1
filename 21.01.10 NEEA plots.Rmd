...
site_info <- read_csv("/Users/jessicakeast/Downloads/RBSA-II-Combined-Database/SiteOneLine.csv")
...
heating2 <- read_csv("heating2.csv")
...
sites %>% filter(ee_site_id %in% jan_properties) %>% 
  left_join(site_info, by = c("site_id" = "CK_SiteID")) %>% 
  ggplot(aes(x= `Building Type`)) + geom_bar(stat = "count")

sites %>% filter(ee_site_id %in% jan_properties) %>% 
  left_join(site_info, by = c("site_id" = "CK_SiteID")) %>% 
  ggplot(aes(x= Region)) + geom_bar(stat = "count")

sites %>% filter(ee_site_id %in% jan_properties) %>% 
  left_join(site_info, by = c("site_id" = "CK_SiteID")) %>% 
  ggplot(aes(x= `Conditioned Area`)) + geom_histogram()
...
heating <- heating %>% 
  left_join(sites, by = "ee_site_id") %>% 
  left_join(select(site_info, c("CK_SiteID", "Conditioned Area", "Conditioned Volume")), by = c("site_id" = "CK_SiteID"))
...
#write_csv(heating, "heating.csv")
#write_csv(heating2, "heating2.csv")
```
...
heating2 %>% 
  filter(circuit_label_type_desc %in% c("Ducted Heatpump", "Ductless Heatpump", "Electric Baseboard Heaters", "Electric Furnace", "Gas Furnace (Component)", "Other Zonal Heat")) %>% 
  mutate(kW_per_area = kW_hour/`Conditioned Area`) %>% 
  ggplot() + geom_boxplot(aes(x=circuit_label_type_desc, y=kW_per_area, color=circuit_label_type_desc))+
  facet_wrap(~month)+
  scale_color_manual(values = wes_palette("Darjeeling1", 6, type = "continuous"),
                    labels = c("Ducted Heatpump (34 sites)", "Ductless Heatpump (9 sites)", "Electric Baseboard Heaters (16 sites)", "Electric Furnace (39 sites)", "Gas Furnance (Fan Energy) (24 sites)", "Other Zonal Heat (11 sites)"))+
  ggplot2::theme_bw() +
  ggplot2::theme(text = ggplot2::element_text(family = choose_font(c("Muli", "sans"))),
          ...
          axis.text.x = element_blank(),
          axis.ticks.x = element_blank())+
  labs(title = "2019 EULR Power Used by Heaters", y = "Load/Energy (kWh/sqft)", x = NULL, color = NULL)
```
...
heating2 %>% 
  ...
  mutate(kW_per_area = kW_hour/`Conditioned Area`) %>% 
  group_by(month, circuit_label_type_desc) %>% 
  summarize(mean_kW_per_area = mean(kW_per_area)) %>% 
  ggplot() + geom_bar(aes(x=circuit_label_type_desc, y=mean_kW_per_area, fill=circuit_label_type_desc), stat = "identity")+
  facet_wrap(~month)+
  scale_fill_manual(values = wes_palette("Darjeeling1", 6, type = "continuous"),
  ...
  ggplot2::theme(text = ggplot2::element_text(family = choose_font(c("Muli", "sans"))),
          ...
          axis.text.x = element_blank(),
          axis.ticks.x = element_blank())+
  labs(title = "2019 EULR Average Power Used by Heaters (Same Sites)", y = "Load/Energy (kWh/sqft)", x = NULL, fill = NULL, caption = "*Ducted heatpumps are used for air conditioning at some sites,\nexplaining the higher load/energy in the Summer months.")
...
heating2 %>% 
  ...
  mutate(kW_per_area = kW_hour/`Conditioned Area`) %>% 
  group_by(month, circuit_label_type_desc) %>% 
  summarize(median_kW_per_area = median(kW_per_area)) %>% 
  ggplot() + geom_bar(aes(x=circuit_label_type_desc, y=median_kW_per_area, fill=circuit_label_type_desc), stat = "identity")+
  facet_wrap(~month)+
  scale_fill_manual(values = wes_palette("Darjeeling1", 6, type = "continuous"),
                    labels = c("Ducted Heatpump (34 sites)*", "Ductless Heatpump (9 sites)", "Electric Baseboard Heaters (16 sites)", "Electric Furnace (39 sites)", "Gas Furnance (Component) (24 sites)", "Other Zonal Heat (11 sites)"))+
  ggplot2::theme_bw() +
  ggplot2::theme(text = ggplot2::element_text(family = choose_font(c("Muli", "sans"))),
          ...
          axis.text.x = element_blank(),
          axis.ticks.x = element_blank())+
  labs(title = "2019 EULR Median Power Used by Heaters (Same Sites)", y = "Load/Energy (kWh/sqft)", x = NULL, fill = NULL, caption = "*Ducted heatpumps are used for air conditioning at some sites,\nexplaining the higher load/energy in the Summer months.")
```
...
```{r}
trial_site <- jan_properties[4]

temp_heating_one_site <- NULL

for (month in mon_list){
  print(month)
  temp_heating_one_site <- read_month_data(month, "/temperature/") %>% 
    filter(ee_site_id == trial_site) %>% 
    left_join(temp_points, by = c("ee_site_id", "transmitter_code")) %>% 
    left_join(sites, by = c("ee_site_id")) %>% 
    mutate(min_t = min_t + hours(tz_utc_offset),
      month=str_extract(min_t, "(?<=-)[0-9]{2}"), 
      hour = parse_number(stringr::str_extract(min_t,"[0-9]{2}(?=:)")),
      date = lubridate::ymd(stringr::str_extract(min_t, "[0-9\\-]*(?= )"))) %>% 
    group_by(ee_site_id, date, hour, month, hz, cz) %>% 
    summarize(mean_outdoor_temp = mean(na.omit(temp[interior_exterior==2])), 
              median_outdoor_temp = median(na.omit(temp[interior_exterior==2])), 
              max_outdoor_temp=max(na.omit(temp[interior_exterior==2])), 
              min_outdoor_temp=min(na.omit(temp[interior_exterior==2])),
              ...
              max_indoor_temp = max(na.omit(temp[interior_exterior==1])),
              min_indoor_temp = min(na.omit(temp[interior_exterior==1]))) %>% 
    rbind(temp_heating_one_site) 
}

temp_heating_one_site <- temp_heating_one_site %>% 
    mutate(month = case_when(
      month == "01" ~ "Jan",
      month == "02" ~ "Feb",
      month == "03" ~ "Mar",
...
temp_heating_one_site <- temp_heating_one_site %>% 
  left_join(heating2, by = c("ee_site_id", "date", "month", "hour"))

ggplot(temp_heating_one_site) + geom_point(aes(x = mean_outdoor_temp, y = kW_hour), alpha = .2) +
  facet_wrap(~circuit_label_type_desc)+
  ggplot2::theme_bw() +
  ggplot2::theme(text = ggplot2::element_text(family = choose_font(c("Muli", "sans"))),
          plot.title=ggplot2::element_text(family = choose_font(c("Roboto Slab", "serif")), face="bold")) +
  labs(x = "Outdoor Temperature (F)", y = "Load/Energy (kWh)", title = "Temperature vs Heating Load for EULR Site 1016", caption = "Each point reflects the temperature and load, respectively, for one hour. Data shown is year-round from 2019.")
...
for (month in mon_list){
  print(month)
  temp_heating <- read_month_data(month, "/temperature/") %>% 
    filter(ee_site_id %in% jan_properties) %>% 
    left_join(temp_points, by = c("ee_site_id", "transmitter_code")) %>% 
    filter(interior_exterior ==2) %>% 
    left_join(sites, by = c("ee_site_id")) %>% 
    mutate(min_t = min_t + hours(tz_utc_offset),
      month=str_extract(min_t, "(?<=-)[0-9]{2}"), 
      hour = parse_number(stringr::str_extract(min_t,"[0-9]{2}(?=:)")),
      date = lubridate::ymd(stringr::str_extract(min_t, "[0-9\\-]*(?= )"))) %>% 
    group_by(ee_site_id, date, hour, month, hz, cz) %>% 
    summarize(mean_outdoor_temp = mean(na.omit(temp)), 
              median_outdoor_temp = median(na.omit(temp)), 
              max_outdoor_temp=max(na.omit(temp)), 
              min_outdoor_temp=min(na.omit(temp))) %>% 
    rbind(temp_heating) 
}

temp_heating <- temp_heating %>% 
  left_join(heating, by = c("ee_site_id", "date", "month", "hour"))
...
temp_heating <- temp_heating %>% 
  mutate(temp_cat = case_when(
    ...
    kWh_by_area = kW_hour/`Conditioned Area`,
    facet_label = case_when(
      circuit_label_type_desc == "Ducted Heatpump" ~ "Ducted Heatpump (34 sites)",
      circuit_label_type_desc == "Ductless Heatpump" ~ "Ductless Heatpump (9 sites)",
      circuit_label_type_desc == "Electric Baseboard Heaters" ~ "Electric Baseboard Heaters (16 sites)",
      circuit_label_type_desc == "Electric Furnace" ~ "Electric Furnace (39 sites)",
      circuit_label_type_desc == "Gas Furnace (Component)" ~ "Gas Furnance (Fan Energy) (24 sites)",
      circuit_label_type_desc == "Other Zonal Heat" ~ "Other Zonal Heat (11 sites)"
    ))
...
temp_heating %>% 
  filter(!is.na(circuit_label_type_desc)) %>% 
  #filter(kW_hour > 0.1) %>% 
  ggplot(aes(x=temp_cat, y = kWh_by_area))+
  geom_boxplot(color = "#257BA0", size = .75)+
  ...
  ggplot2::theme(text = ggplot2::element_text(family = choose_font(c("Muli", "sans"))),
          plot.title=ggplot2::element_text(family = choose_font(c("Roboto Slab", "serif")), face="bold"))+
  labs(title = "Power Used by Heaters by Temperature", y = "Load/Energy (kWh/sqft)", x = "Temperature (F)") 

temp_heating %>% 
  filter(circuit_label_type_desc %in% c("Ducted Heatpump", "Ductless Heatpump", "Electric Baseboard Heaters", "Electric Furnace")) %>% 
  #filter(kW_hour > 0.1) %>% 
  ggplot(aes(x=temp_cat, y = kWh_by_area))+
  geom_boxplot(color = "#257BA0", size = .75)+
  facet_wrap(~facet_label, nrow = 2)+
...
temp_heating %>% 
  filter(circuit_label_type_desc %in% c("Ducted Heatpump", "Ductless Heatpump")) %>% 
  #filter(kW_hour > 0.1) %>% 
  ggplot(aes(x=temp_cat, y = kWh_by_area))+
  geom_boxplot(color = "#257BA0", size = .75)+
  facet_wrap(~facet_label, nrow = 2)+
